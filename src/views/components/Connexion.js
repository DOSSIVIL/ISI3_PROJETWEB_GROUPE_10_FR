    ...
    try {
        ...
        switch(mode) {
            ...
            case 'resetPassword':
                ...
                showNotification(`Réinitialisation du mot de passe pour ${email}`, 'info');
        ...
        setTimeout(() => {
            window.location.href = '/src/views/templates/Connexion.html?verified=true';
        }, 2000);
...
async function loginUser(email, password) {
    try {
        ...
        if (!user.emailVerified) {
            await auth.signOut();
            showNotification('❌ Email non vérifié. Vérifiez votre boîte mail.', 'error');
...
async function resendVerification(email) {
    try {
        ...
        if (user && user.email === email) {
            ...
            showNotification('❌ Connectez-vous d\'abord pour renvoyer la vérification', 'error');
            return;
        }
        
        showNotification('📧 Email de vérification renvoyé ! Vérifiez votre boîte mail.', 'success');
    } catch (error) {
...
function setupSocialButtons() {
    // Bouton Google
    const googleBtn = document.querySelector('button[class*="flex items-center justify-center gap-3"]');
    if (googleBtn) {
...
function checkDirectAccess() {
    ...
    if (!userData) {
        const redirectUrl = `/connexion?error=session_required&redirect=${encodeURIComponent(window.location.pathname)}`;
...
async function handleLogin(e) {
    ...
    try {
        ...
        if (userProfile) {
            const displayName = userProfile.display_name || userProfile.prenom || user.email.split('@')[0];
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    try {
        ...
        if (submitBtn) {
            submitBtn.disabled = true;
            submitBtn.innerHTML = '<i class="fas fa-exclamation-triangle mr-2"></i> Service temporairement indisponible';
            submitBtn.classList.add('bg-red-500', 'cursor-not-allowed');
            showNotification('Le service d\'authentification est en maintenance', 'error');
        }
    }
