...
async function checkUserRegistration(userId) {
  try {
    ...
    console.log(
      "Utilisateur non trouvé dans 'users', recherche dans collections spécifiques..."
    );

    // Si pas dans users, vérifier dans les collections spécifiques
    const [etudiantDoc, enseignantDoc] = await Promise.all([
      db.collection("etudiants").where("uid", "==", userId).limit(1).get(),
      db.collection("enseignants").where("uid", "==", userId).limit(1).get(),
    ]);
...
async function updateLastLogin(userId, registration) {
  try {
    ...
    if (registration.collection && registration.docId) {
      ...
      console.log(
        `Dernière connexion mise à jour dans ${registration.collection}`
      );
    }
...
if (eyeToggle) {
  eyeToggle.addEventListener("click", function () {
    ...
    this.className = isPassword
      ? "fas fa-eye-slash eye-toggle absolute right-4 top-4 text-gray-400"
...
function showError(fieldId, message) {
  ...
  if (!errorElement || !errorElement.classList.contains("error-message")) {
    ...
    errorElement.className =
      "mt-2 text-xs font-medium text-red-500 error-message";
...
async function handleLogin(email, password) {
  try {
    ...
    if (!registration) {
      ...
      await auth.signOut();
      showNotification(
        "Vous n'êtes pas inscrit dans le système. Veuillez d'abord vous inscrire.",
        "error"
      );
      return false;
    }
    ...
    showNotification(
      `Connexion réussie ! Bienvenue ${welcomeText} (${welcomeMessage}). Redirection...`,
      "success"
    );
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  auth.onAuthStateChanged(async (user) => {
    if (user) {
      ...
      if (
        registration &&
        (registration.role === "enseignant" || registration.role === "etudiant")
      ) {
        // Rediriger selon le rôle
        let redirectPage = "";
        if (registration.role === "enseignant") {
          redirectPage = "../../templates/tuteurDashboard.html";
        } else if (registration.role === "etudiant") {
          redirectPage = "../../templates/etudiantDasboard.html";
        }

        if (redirectPage) {
          console.log(
            `Utilisateur déjà connecté (${registration.role}), redirection vers ${redirectPage}...`
          );

          // Afficher un message avant redirection
          showNotification(
            `Vous êtes déjà connecté en tant que ${registration.role}. Redirection...`,
            "success"
          );
        ...
        await auth.signOut();
        showNotification(
          "Votre compte n'est pas inscrit dans le système. Veuillez vous inscrire d'abord.",
          "error"
        );
      }
    } else {
  ...
  socialButtons.forEach((button) => {
    button.addEventListener("click", function () {
      ...
      showNotification(
        `Connexion avec ${platform} n'est pas encore disponible. Utilisez la connexion par email.`,
        "error"
      );
    });
  });
  ...
  if (googleButton) {
    googleButton.addEventListener("click", function () {
      showNotification(
        "Connexion avec Google n'est pas encore disponible. Utilisez la connexion par email.",
        "error"
      );
    });
  }
  ...
  if (forgotPasswordLink) {
    forgotPasswordLink.addEventListener("click", function (e) {
      e.preventDefault();
      showNotification(
        "La réinitialisation du mot de passe n'est pas encore disponible. Contactez l'administrateur.",
        "error"
      );
    });
  }
...
window.addEventListener("offline", function () {
  showNotification(
    "Vous avez perdu la connexion internet. Certaines fonctionnalités peuvent être limitées.",
    "error"
  );
});
