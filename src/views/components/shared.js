// scripts/mock-data.js
// Données simulées pour le développement

const MockData = {
    // Tuteurs disponibles
    tutors: [
        {
            id: 1,
            name: "Dr. Marie Curie",
            email: "marie.curie@educonnect.africa",
            subject: "physics",
            subjectName: "Physique",
            rating: 4.9,
            totalSessions: 128,
            pricePerHour: 2500,
            available: true,
            online: true,
            description: "Spécialiste en physique quantique avec 10 ans d'expérience. Lauréate du prix Nobel de physique.",
            languages: ["Français", "Anglais", "Polonais"],
            education: "PhD en Physique - Université de Paris",
            experience: "10 ans",
            avatar: "MC",
            reviews: 47,
            specialties: ["Mécanique quantique", "Radioactivité", "Physique nucléaire"],
            schedule: {
                monday: ["14:00-16:00", "18:00-20:00"],
                tuesday: ["10:00-12:00", "15:00-17:00"],
                wednesday: ["09:00-11:00", "14:00-16:00"],
                thursday: ["13:00-15:00", "17:00-19:00"],
                friday: ["11:00-13:00", "16:00-18:00"]
            }
        },
        {
            id: 2,
            name: "Prof. Jean Dupont",
            email: "jean.dupont@educonnect.africa",
            subject: "math",
            subjectName: "Mathématiques",
            rating: 4.7,
            totalSessions: 95,
            pricePerHour: 2000,
            available: true,
            online: false,
            description: "Expert en algèbre et calcul différentiel. Passionné par la pédagogie adaptative.",
            languages: ["Français"],
            education: "Master en Mathématiques - École Normale Supérieure",
            experience: "8 ans",
            avatar: "JD",
            reviews: 32,
            specialties: ["Algèbre linéaire", "Calcul différentiel", "Statistiques"],
            schedule: {
                monday: ["08:00-10:00", "14:00-16:00"],
                tuesday: ["09:00-11:00", "15:00-17:00"],
                wednesday: ["10:00-12:00", "16:00-18:00"],
                thursday: ["13:00-15:00"],
                friday: ["11:00-13:00", "17:00-19:00"]
            }
        },
        {
            id: 3,
            name: "Alan Turing",
            email: "alan.turing@educonnect.africa",
            subject: "info",
            subjectName: "Informatique",
            rating: 5.0,
            totalSessions: 210,
            pricePerHour: 3000,
            available: true,
            online: true,
            description: "Pionnier de l'informatique moderne. Expert en algorithmes et cryptographie.",
            languages: ["Anglais", "Français"],
            education: "Computer Science - Massachusetts Institute of Technology",
            experience: "12 ans",
            avatar: "AT",
            reviews: 89,
            specialties: ["Algorithmes", "Cryptographie", "Intelligence Artificielle"],
            schedule: {
                monday: ["10:00-12:00", "15:00-17:00"],
                tuesday: ["14:00-16:00", "18:00-20:00"],
                wednesday: ["09:00-11:00", "13:00-15:00"],
                thursday: ["11:00-13:00", "16:00-18:00"],
                friday: ["10:00-12:00", "14:00-16:00"]
            }
        },
        {
            id: 4,
            name: "Chinua Achebe",
            email: "chinua.achebe@educonnect.africa",
            subject: "literature",
            subjectName: "Littérature",
            rating: 4.8,
            totalSessions: 75,
            pricePerHour: 1800,
            available: false,
            online: true,
            description: "Romancier et poète nigérian. Spécialiste en littérature africaine postcoloniale.",
            languages: ["Anglais", "Français", "Igbo"],
            education: "PhD en Littérature - Université de Ibadan",
            experience: "15 ans",
            avatar: "CA",
            reviews: 28,
            specialties: ["Littérature africaine", "Postcolonialisme", "Roman"],
            schedule: {
                monday: ["13:00-15:00"],
                tuesday: ["10:00-12:00", "16:00-18:00"],
                wednesday: ["14:00-16:00"],
                thursday: ["09:00-11:00", "15:00-17:00"],
                friday: ["11:00-13:00"]
            }
        },
        {
            id: 5,
            name: "Wangari Maathai",
            email: "wangari.maathai@educonnect.africa",
            subject: "science",
            subjectName: "Sciences de la Terre",
            rating: 4.6,
            totalSessions: 62,
            pricePerHour: 2200,
            available: true,
            online: false,
            description: "Environnementaliste et biologiste kenyane. Lauréate du prix Nobel de la paix.",
            languages: ["Anglais", "Swahili"],
            education: "PhD en Sciences Environnementales - Université de Nairobi",
            experience: "20 ans",
            avatar: "WM",
            reviews: 19,
            specialties: ["Écologie", "Biologie végétale", "Développement durable"],
            schedule: {
                monday: ["09:00-11:00"],
                tuesday: ["14:00-16:00"],
                wednesday: ["10:00-12:00", "15:00-17:00"],
                thursday: ["13:00-15:00"],
                friday: ["11:00-13:00", "16:00-18:00"]
            }
        },
        {
            id: 6,
            name: "Albert Einstein",
            email: "albert.einstein@educonnect.africa",
            subject: "physics",
            subjectName: "Physique",
            rating: 4.9,
            totalSessions: 300,
            pricePerHour: 3500,
            available: true,
            online: true,
            description: "Physicien théoricien. Auteur de la théorie de la relativité.",
            languages: ["Allemand", "Anglais", "Français"],
            education: "PhD en Physique - ETH Zurich",
            experience: "30 ans",
            avatar: "AE",
            reviews: 156,
            specialties: ["Relativité", "Physique théorique", "Cosmologie"],
            schedule: {
                monday: ["14:00-16:00"],
                tuesday: ["10:00-12:00", "15:00-17:00"],
                wednesday: ["09:00-11:00"],
                thursday: ["13:00-15:00", "17:00-19:00"],
                friday: ["11:00-13:00"]
            }
        },
        {
            id: 7,
            name: "Ada Lovelace",
            email: "ada.lovelace@educonnect.africa",
            subject: "info",
            subjectName: "Informatique",
            rating: 4.8,
            totalSessions: 85,
            pricePerHour: 2800,
            available: true,
            online: true,
            description: "Pionnière de la programmation informatique. Mathématicienne et écrivaine.",
            languages: ["Anglais"],
            education: "Mathématiques - Université de Cambridge",
            experience: "8 ans",
            avatar: "AL",
            reviews: 41,
            specialties: ["Algorithmes", "Programmation", "Mathématiques appliquées"],
            schedule: {
                monday: ["10:00-12:00", "15:00-17:00"],
                tuesday: ["14:00-16:00"],
                wednesday: ["09:00-11:00", "13:00-15:00"],
                thursday: ["11:00-13:00"],
                friday: ["10:00-12:00", "14:00-16:00"]
            }
        },
        {
            id: 8,
            name: "Léonard de Vinci",
            email: "leonard.davinci@educonnect.africa",
            subject: "science",
            subjectName: "Sciences",
            rating: 4.9,
            totalSessions: 120,
            pricePerHour: 3200,
            available: false,
            online: false,
            description: "Polymathe de la Renaissance. Expert en anatomie, ingénierie et arts.",
            languages: ["Italien", "Français"],
            education: "Autodidacte",
            experience: "40 ans",
            avatar: "LV",
            reviews: 67,
            specialties: ["Anatomie", "Ingénierie", "Arts visuels"],
            schedule: {
                monday: ["13:00-15:00"],
                tuesday: ["10:00-12:00"],
                wednesday: ["14:00-16:00"],
                thursday: ["09:00-11:00"],
                friday: ["11:00-13:00", "15:00-17:00"]
            }
        }
    ],

    // Matières disponibles
    subjects: [
        { id: "math", name: "Mathématiques", icon: "fa-calculator", color: "#FFD700" },
        { id: "physics", name: "Physique", icon: "fa-atom", color: "#FFA500" },
        { id: "info", name: "Informatique", icon: "fa-laptop-code", color: "#089182" },
        { id: "literature", name: "Littérature", icon: "fa-book", color: "#8B3A9E" },
        { id: "science", name: "Sciences de la Terre", icon: "fa-globe-africa", color: "#1A3B52" },
        { id: "chemistry", name: "Chimie", icon: "fa-flask", color: "#8B7355" }
    ],

    // Messages de chat pré-définis
    chatMessages: [
        {
            id: 1,
            sender: "tutor",
            name: "Dr. Marie Curie",
            time: "10:00",
            content: "Bonjour ! Bienvenue dans notre session de tutorat. Comment puis-je vous aider aujourd'hui ?",
            type: "text"
        },
        {
            id: 2,
            sender: "student",
            name: "Vous",
            time: "10:01",
            content: "Bonjour Dr. Curie ! J'ai des difficultés avec les équations du mouvement rectiligne uniforme.",
            type: "text"
        },
        {
            id: 3,
            sender: "tutor",
            name: "Dr. Marie Curie",
            time: "10:02",
            content: "Excellent sujet ! Commençons par la formule de base : v = d/t. Avez-vous des exercices spécifiques ?",
            type: "text"
        },
        {
            id: 4,
            sender: "tutor",
            name: "Dr. Marie Curie",
            time: "10:03",
            content: "Je viens de partager un document PDF avec des exercices pratiques. Regardez dans les fichiers partagés.",
            type: "file",
            fileInfo: {
                name: "Exercices_MRU.pdf",
                size: "2.4 MB",
                type: "pdf"
            }
        },
        {
            id: 5,
            sender: "student",
            name: "Vous",
            time: "10:04",
            content: "Merci ! Je vois le fichier. Pour la première équation, je dois trouver la vitesse...",
            type: "text"
        },
        {
            id: 6,
            sender: "tutor",
            name: "Dr. Marie Curie",
            time: "10:05",
            content: "Exactement. Souvenez-vous que dans un MRU, la vitesse est constante. Utilisons les données de l'exercice 1.",
            type: "text"
        },
        {
            id: 7,
            sender: "system",
            name: "Système",
            time: "10:06",
            content: "Le tuteur a partagé son écran",
            type: "system"
        },
        {
            id: 8,
            sender: "student",
            name: "Vous",
            time: "10:07",
            content: "Ah, je comprends mieux maintenant avec le tableau blanc. Merci beaucoup !",
            type: "text"
        }
    ],

    // Sessions de tutorat actives
    activeSessions: [
        {
            id: "session_001",
            tutorId: 1,
            tutorName: "Dr. Marie Curie",
            studentName: "Étudiant Demo",
            subject: "physics",
            startTime: "2024-11-25T10:00:00",
            duration: 45,
            status: "active"
        },
        {
            id: "session_002",
            tutorId: 3,
            tutorName: "Alan Turing",
            studentName: "John Doe",
            subject: "info",
            startTime: "2024-11-25T11:30:00",
            duration: 60,
            status: "active"
        }
    ],

    // Statistiques
    statistics: {
        totalTutors: 45,
        onlineTutors: 12,
        totalStudents: 320,
        activeSessions: 8,
        completedSessions: 1247,
        averageRating: 4.7,
        subjectsCovered: 6
    },

    // Fonctions utilitaires
    getTutorById(id) {
        return this.tutors.find(tutor => tutor.id === id);
    },

    getTutorsBySubject(subject) {
        if (subject === 'all') return this.tutors;
        return this.tutors.filter(tutor => tutor.subject === subject);
    },

    getAvailableTutors() {
        return this.tutors.filter(tutor => tutor.available);
    },

    getOnlineTutors() {
        return this.tutors.filter(tutor => tutor.online);
    },

    getSubjectInfo(subjectId) {
        return this.subjects.find(subject => subject.id === subjectId);
    },

    generateMockReview(rating) {
        const reviews = [
            "Excellent tuteur, très patient et compétent !",
            "Explications claires et précises, je recommande !",
            "A m'aide à comprendre des concepts difficiles.",
            "Très professionnel et à l'écoute.",
            "Sessions très productives, merci !",
            "Approche pédagogique adaptée à mes besoins.",
            "Disponible et réactif aux questions.",
            "Matériel pédagogique de qualité.",
            "A amélioré significativement mes résultats.",
            "Je continue les sessions avec plaisir."
        ];
        return reviews[Math.floor(Math.random() * reviews.length)];
    },

    // Simuler une recherche
    searchTutors(query) {
        const lowerQuery = query.toLowerCase();
        return this.tutors.filter(tutor => 
            tutor.name.toLowerCase().includes(lowerQuery) ||
            tutor.subjectName.toLowerCase().includes(lowerQuery) ||
            tutor.description.toLowerCase().includes(lowerQuery) ||
            tutor.specialties.some(s => s.toLowerCase().includes(lowerQuery))
        );
    },

    // Obtenir les messages pour une session
    getSessionMessages(sessionId) {
        // Pour la démo, retourner tous les messages
        return this.chatMessages;
    },

    // Ajouter un nouveau message
    addChatMessage(sender, content, type = 'text') {
        const newMessage = {
            id: this.chatMessages.length + 1,
            sender: sender,
            name: sender === 'student' ? 'Vous' : this.getTutorById(1).name,
            time: new Date().toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }),
            content: content,
            type: type
        };
        this.chatMessages.push(newMessage);
        return newMessage;
    }
};

// Exposer les données globalement pour le développement
window.MockData = MockData;

// Exporter pour Node.js
if (typeof module !== 'undefined' && module.exports) {
    module.exports = MockData;
}