...
function checkPasswordStrength() {
    ...
    if (reqLength) reqLength.className = hasLength ? 'fas fa-check-circle requirement-icon requirement-met' : 'fas fa-times-circle requirement-icon requirement-not-met';
    if (reqUppercase) reqUppercase.className = hasUppercase ? 'fas fa-check-circle requirement-icon requirement-met' : 'fas fa-times-circle requirement-icon requirement-not-met';
    if (reqLowercase) reqLowercase.className = hasLowercase ? 'fas fa-check-circle requirement-icon requirement-met' : 'fas fa-times-circle requirement-icon requirement-not-met';
    if (reqNumber) reqNumber.className = hasNumber ? 'fas fa-check-circle requirement-icon requirement-met' : 'fas fa-times-circle requirement-icon requirement-not-met';
    if (reqSpecial) reqSpecial.className = hasSpecial ? 'fas fa-check-circle requirement-icon requirement-met' : 'fas fa-times-circle requirement-icon requirement-not-met';
...
function validateForm() {
    ...
    if (passwordScore) {
        ...
        if (score < 60) {
            errors.push({
                field: 'password',
                message: 'Le mot de passe est trop faible. Améliorez-le pour continuer.'
            });
        }
    }
...
function displayFormErrors(errors) {
    errors.forEach(error => {
        ...
        if (!errorElement) {
            errorElement = fieldElement.parentElement.parentElement.querySelector('.error-message');
        }
...
async function handleFormSubmit(e) {
    ...
    if (!db || !auth) {
        showNotification('Erreur de connexion à la base de données. Veuillez rafraîchir la page.', 'error');
        return;
    }
    ...
    try {
        ...
        const formData = {
            ...
            telephone: document.getElementById('countryCode').value + document.getElementById('telephone').value.replace(/\D/g, '')
        };
        ...
        setTimeout(() => {
            ...
            if (passwordStrengthText) passwordStrengthText.textContent = 'Force du mot de passe';
...
function updateStepIndicator(activeStep) {
    ...
    document.querySelectorAll('.step').forEach((step, index) => {
        if (index <= activeStep) {
            ...
            step.querySelector('div').classList.remove('bg-gradient-to-r', 'from-blue-600', 'to-blue-700', 'text-white');
